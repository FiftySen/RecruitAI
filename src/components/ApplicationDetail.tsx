    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-cyan-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid gap-8">
          ...
          {/* Assessment Details - Show what will be evaluated */}
          {application.position?.assessmentConfiguration && (
            application.position.assessmentConfiguration.selectedSoftSkillsSubAreas.length > 0 || 
            application.position.assessmentConfiguration.selectedTechnicalSubSkills.length > 0 ||
            application.position.assessmentConfiguration.customSoftSkills.trim() ||
            application.position.assessmentConfiguration.customTechnicalSkills.trim()
          ) && (
            <Card className="bg-gradient-to-r from-indigo-50 to-purple-50 border-indigo-200 shadow-lg">
              <CardHeader>
                ...
                <CardDescription className="text-indigo-700">
                  Detailed breakdown of skills that will be evaluated in your assessments
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  {(application.position.assessmentConfiguration.selectedSoftSkillsSubAreas.length > 0 ||
                    ...
                    <div className="space-y-3">
                      ...
                      {application.position.assessmentConfiguration.selectedSoftSkillsSubAreas.length > 0 && (
                        <div className="space-y-2">
                          <div className="text-sm text-blue-700">Specific Skills:</div>
                          <div className="flex flex-wrap gap-1">
                            {application.position.assessmentConfiguration.selectedSoftSkillsSubAreas.map((skill, index) => (
                              <Badge key={index} variant="outline" className="text-xs bg-blue-100 border-blue-300 text-blue-800">
                                {skill}
                              </Badge>
                            ))}
                      ...
                      {application.position.assessmentConfiguration.customSoftSkills.trim() && (
                        <div className="space-y-2">
                          <div className="text-sm text-blue-700">Additional Custom Skills:</div>
                          <div className="flex flex-wrap gap-1">
                            {application.position.assessmentConfiguration.customSoftSkills.split('\n')
                              .filter(skill => skill.trim()).map((skill, index) => (
                              <Badge key={index} variant="outline" className="text-xs bg-purple-100 border-purple-300 text-purple-800">
                                {skill.trim()}
                              </Badge>
                            ))}
                          </div>
                  ...
                  {(application.position.assessmentConfiguration.selectedTechnicalSubSkills.length > 0 ||
                    ...
                    <div className="space-y-3">
                      ...
                      {application.position.assessmentConfiguration.selectedTechnicalSubSkills.length > 0 && (
                        <div className="space-y-2">
                          <div className="text-sm text-green-700">Specific Skills:</div>
                          <div className="flex flex-wrap gap-1">
                            {application.position.assessmentConfiguration.selectedTechnicalSubSkills.map((skill, index) => (
                              <Badge key={index} variant="outline" className="text-xs bg-green-100 border-green-300 text-green-800">
                                {skill}
                              </Badge>
                            ))}
                      ...
                      {application.position.assessmentConfiguration.customTechnicalSkills.trim() && (
                        <div className="space-y-2">
                          <div className="text-sm text-green-700">Additional Custom Skills:</div>
                          <div className="flex flex-wrap gap-1">
                            {application.position.assessmentConfiguration.customTechnicalSkills.split('\n')
                              .filter(skill => skill.trim()).map((skill, index) => (
                              <Badge key={index} variant="outline" className="text-xs bg-purple-100 border-purple-300 text-purple-800">
                                {skill.trim()}
                              </Badge>
                ...
                <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
                  <h4 className="font-semibold mb-2 text-amber-800">💡 Assessment Preparation Tips:</h4>
                  <div className="text-sm text-amber-700 space-y-1">
                    <p>• Study the specific skills listed above - questions will focus on these areas</p>
                    <p>• Prepare concrete examples from your experience for each skill</p>
                    <p>• Practice explaining your problem-solving approach clearly</p>
                    <p>• Be ready to discuss real projects and challenges you've faced</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                ...
                <div className="grid grid-cols-3 gap-6 text-center mb-6">
                  ...
                  <div className="p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                    <div className="text-2xl font-bold text-indigo-600">
                      {application.scores.weighted}%
                    </div>
                    <div className="text-sm text-indigo-600">Overall Score</div>
                  </div>
                </div>
                <div className="text-center">
                  <Button variant="outline" onClick={() => onNavigate('results')} className="border-indigo-300 text-indigo-700 hover:bg-indigo-100">
