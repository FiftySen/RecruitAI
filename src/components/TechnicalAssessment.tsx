// src/components/TechnicalAssessment.tsx
...
export function TechnicalAssessment({ onBack }: { onBack: () => void }) { // <-- ADD onBack PROP
  const { user } = useAuth();
    ...
    const startAssessment = async () => {
      ...
      try {
        ...
        const response = await fetch('https://kjksyigqsvrpowxodnim.supabase.co/functions/v1/make-server-6ead2a10/start-assessment', {
          ...
          body: JSON.stringify({ userId: user.id, positionId, assessmentType: 'technical' }),
        });
